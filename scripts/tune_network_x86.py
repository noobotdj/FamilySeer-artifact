...
#        model_file = "./models/%s-%s-B%d-%s.tar" % (args.model, layout, batch_size, target.kind.name)
        ...
        log_file = "%s-%s-B%d-%s.json" % (network, layout, batch_size, target.kind.name)
        model_file = "./models/%s-%s-B%d-%s.tar" % (network, layout, batch_size, target.kind.name)
    elif args.model == 'graph_generation_opt':
...
batch_size = 1
layout = "NHWC"
target = tvm.target.Target(avxinfo())
dtype = "float32"
log_file = "%s-%s-B%d-%s.json" % (network, layout, batch_size, target.kind.name)
model_file = "./models/%s-%s-B%d-%s.tar" % (network, layout, batch_size, target.kind.name)                                                                                          
get_name()
...
def run_tuning():
    ...
    tune_option = auto_scheduler.TuningOptions(
        num_measure_trials=800 * (len(tasks) + 1),  # change this to 20000 to achieve the best performance
